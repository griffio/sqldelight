...
/**
 * Operators '&&' | '<<' | '>>' | '&>' | '&<' | '-|-' for TsRange, TsTzRange
 */
internal abstract class RangeOperatorExpressionMixin(node: ASTNode) :
  ...
  override fun annotate(annotationHolder: SqlAnnotationHolder) {
    ...
    when (columnType) {
      null, "TSRANGE", "TSTZRANGE" -> super.annotate(annotationHolder)
      else -> annotationHolder.createErrorAnnotation(firstChild.firstChild, "expression must be TSRANGE, TSTZRANGE.")
    }
